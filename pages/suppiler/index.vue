            ...
            <v-card>
              ...
              <v-card-actions>
                ...
                <v-btn
                  color="blue darken-1"
                  text
                  @click="deleteItemConfirm(editedItem.sp_id)"
                  >ຕົກລົງ</v-btn
                >
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1
        ? 'ເພີ່ມ ຂໍ້ມູນຜູ້ສະຫນອງ'
        : 'ແກ້ໄຂ ຂໍ້ມູນຜູ້ສະຫນອງ'
    },
  },
  ...
  methods: {
    ...
    async save() {
      if (this.editedIndex > -1) {
        const sendresdata = {
          sp_name: this.editedItem.sp_name,
        }

        try {
          await this.$axios
            .put(`/suppilers/${this.editedItem.sp_id}`, sendresdata)
            .then((res) => {
        ...
        try {
          await this.$axios.post('/suppilers', getresdata).then((res) => {
